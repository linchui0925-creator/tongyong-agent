...
uniform float uEyeClosed; // 新增：是否闭眼，0睁眼，1完全闭眼
uniform float uGlowBoost; // 新增：微光增强
uniform float uHoverPulse; // 新增： hover时的脉冲效果
...
void main() {
  ...
  outerEyeGlow *= uGlowIntensity * (1.0 + uGlowBoost * 0.5 + uHoverPulse * 0.3);
  ...
  outerBgGlow *= 0.15 * (1.0 + uGlowBoost);

  // 闭眼时加一条眼睑缝隙的光
  float eyelidLine = smoothstep(0.01, 0.0, abs(uv.y)) * uEyeClosed * 0.6;
  float eyelidGlow = smoothstep(0.08, 0.0, abs(uv.y)) * uEyeClosed * 0.3;

  vec3 color = uEyeColor * uIntensity * clamp(max(innerRing + innerEye, outerEyeGlow + outerBgGlow) - pupil, 0.0, 3.0);
  // 加眼睑光
  color += vec3(1.0, 0.9, 0.7) * (eyelidLine + eyelidGlow) * (1.0 + uGlowBoost);
  color += uBgColor;
...
interface EvilEyeProps {
  ...
  /** 0 = fully open, 1 = fully closed. Drives a smooth tween. */
  closeAmount?: number;
  glowBoost?: number;
  hoverPulse?: number;
}
...
export default function EvilEye({
  ...
  const programRef = useRef<{ uniforms: { [key: string]: { value: any } } } | null>(null);
  // closeAmount is read inside the rAF render loop without re-creating the GL context.
  const closeAmountRef = useRef(closeAmount);
  closeAmountRef.current = closeAmount;
  ...
  useEffect(() => {
    ...
    program = new Program(gl, {
      ...
      uniforms: {
        ...
        uEyeClosed: { value: closeAmount },
        uGlowBoost: { value: glowBoost },
        uHoverPulse: { value: hoverPulse }
      }
    });
    ...
    function update(time: number) {
      ...
      program.uniforms.uEyeClosed.value = closeAmountRef.current;
  ...
  // Note: uEyeClosed is driven every frame from closeAmountRef inside the rAF loop above,
  // so a separate effect on isClosed is no longer required.

  useEffect(() => {
    if (!programRef.current) return;
    programRef.current.uniforms.uGlowBoost.value = glowBoost;
  }, [glowBoost]);

  useEffect(() => {
    if (!programRef.current) return;
    programRef.current.uniforms.uHoverPulse.value = hoverPulse;
  }, [hoverPulse]);
